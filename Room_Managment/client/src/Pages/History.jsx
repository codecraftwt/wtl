...
const History = () => {
    ...
    return bookingsData.map(booking => {
      ...
      return {
        ...
        guest: {
          ...
          email: booking.userId?.email || 'guest@example.com',
          phone: '+1 (555) 000-0000',
          avatar: `https://ui-avatars.com/api/?name=${booking.userId?.name || 'Guest'}&background=EAB308&color=fff&size=40`,
          level: getGuestLevel(booking.userId?.createdAt)
        },
        owner: {
          name: booking.ownerId?.name || 'Owner',
  ...
  const getStatusColor = (booking) => {
    if (booking.isbookingcancel) return 'rose';
    if (booking.bookingCompleted) return 'emerald';
    if (booking.payment_status) return 'yellow';
    return 'orange';
  };
  ...
  return (
    <div className="flex flex-col min-h-screen bg-gradient-to-br from-gray-50 via-yellow-50 to-red-50 font-display">
      ...
      <main className="flex-1 max-w-[1600px] mx-auto w-full px-6 py-8 space-y-8 relative z-10">
        {/* Header & Summary Stats */}
        <section className="space-y-6">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
          ...
          {/* Stats Grid - Updated to yellow/red theme */}
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            <div className="bg-white p-5 rounded-xl border border-gray-200 shadow-sm">
              <p className="text-gray-500 text-sm font-medium">Completed Stays</p>
              <div className="flex items-end justify-between mt-2">
                <span className="text-3xl font-bold text-gray-900">{stats.completedStays}</span>
            ...
            <div className="bg-white p-5 rounded-xl border border-gray-200 shadow-sm">
              ...
              <div className="flex items-end justify-between mt-2">
                <span className="text-3xl font-bold text-gray-900">{formatCurrency(stats.totalRevenue)}</span>
                <span className="text-xs font-semibold bg-red-100 text-red-700 px-2 py-1 rounded">All time</span>
              </div>
            </div>
            <div className="bg-white p-5 rounded-xl border border-gray-200 shadow-sm">
              <p className="text-gray-500 text-sm font-medium">Cancellation Rate</p>
              <div className="flex items-end justify-between mt-2">
                <span className="text-3xl font-bold text-gray-900">{stats.cancellationRate}%</span>
                <span className="text-xs font-semibold bg-rose-100 text-rose-600 px-2 py-1 rounded">
                  {bookings.filter(b => b.status === 'Cancelled').length} cancelled
                </span>
              </div>
            </div>
            <div className="bg-white p-5 rounded-xl border border-gray-200 shadow-sm">
              <p className="text-gray-500 text-sm font-medium">Avg. Stay Duration</p>
              <div className="flex items-end justify-between mt-2">
                <span className="text-3xl font-bold text-gray-900">{stats.avgStayDuration} Days</span>
                <span className="text-xs font-semibold bg-gradient-to-r from-yellow-100 to-red-100 text-red-600 px-2 py-1 rounded">
                  {bookings.length} bookings
                </span>
              </div>
            </div>
        ...
        {/* Search & Filters - Updated to yellow/red theme */}
        <section className="bg-white p-4 rounded-xl border border-gray-200 shadow-sm">
          <div className="flex flex-wrap items-center gap-4">
            <div className="flex-1 min-w-[300px] relative">
              <span className="material-icons absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 text-lg">search</span>
              <input 
                className="w-full pl-10 pr-4 py-2 bg-gray-50 border border-gray-200 rounded-lg text-sm focus:ring-2 focus:ring-yellow-500/50 focus:border-yellow-500 outline-none" 
                placeholder="Search Guest Name, Booking ID, or Property..." 
                type="text"
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
            ...
            <div className="flex items-center gap-2">
              <div className="relative">
                <select 
                  className="appearance-none bg-gray-50 border border-gray-200 rounded-lg text-sm pl-4 pr-10 py-2 focus:ring-2 focus:ring-yellow-500/50 focus:border-yellow-500 outline-none cursor-pointer"
                  value={selectedStatus}
                  onChange={(e) => setSelectedStatus(e.target.value)}
                >
                  <option>All Statuses</option>
              ...
              <div className="relative">
                <button className="bg-gray-50 border border-gray-200 rounded-lg text-sm px-4 py-2 flex items-center gap-2 hover:bg-gray-100 transition-colors">
                  <span className="material-icons text-sm text-gray-400">calendar_today</span>
                  <span>All Time</span>
                </button>
              </div>
              <button 
                className="text-red-600 text-sm font-semibold hover:text-red-700 px-2"
                onClick={() => {
                  setSearchQuery('');
                  setSelectedStatus('All Statuses');
                  setSelectedRoom('All Rooms');
        ...
        {/* Data Table - Updated to yellow/red theme */}
        <section className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
          {filteredBookings.length === 0 ? (
            <div className="text-center py-12">
              ...
              <p className="text-gray-400 text-sm mt-1">Your past bookings will appear here</p>
            </div>
          ) : (
            <>
              <div className="overflow-x-auto">
                <table className="w-full text-left border-collapse">
                  <thead className="bg-gradient-to-r from-yellow-50 to-red-50 border-b border-gray-200 sticky top-0 z-10">
                    <tr>
                      <th className="py-4 px-6 text-xs font-bold text-gray-500 uppercase tracking-wider">
                        <input className="rounded border-gray-300 text-yellow-500 focus:ring-yellow-500" type="checkbox" />
                      </th>
                      <th className="py-4 px-6 text-xs font-bold text-gray-500 uppercase tracking-wider">Booking ID</th>
                      <th className="py-4 px-6 text-xs font-bold text-gray-500 uppercase tracking-wider">Guest</th>
                      <th className="py-4 px-6 text-xs font-bold text-gray-500 uppercase tracking-wider">Property / Room</th>
                      <th className="py-4 px-6 text-xs font-bold text-gray-500 uppercase tracking-wider">Stay Dates</th>
                      <th className="py-4 px-6 text-xs font-bold text-gray-500 uppercase tracking-wider text-right">Total Price</th>
                  ...
                  <tbody className="divide-y divide-gray-100">
                    {filteredBookings.map((booking, index) => (
                      ...
                      >
                        <td className="py-4 px-6">
                          <input className="rounded border-gray-300 text-yellow-500 focus:ring-yellow-500" type="checkbox" onClick={(e) => e.stopPropagation()} />
                        </td>
                        <td className="py-4 px-6 text-sm font-semibold text-red-600">{booking.id}</td>
                        <td className="py-4 px-6">
                          <div className="flex items-center gap-3">
                            <img 
                            ...
                            />
                            <div>
                              <p className="text-sm font-semibold text-gray-900 leading-none">{booking.guest.name}</p>
                              <p className="text-xs text-gray-500 mt-1">Guest Level: {booking.guest.level}</p>
                            </div>
                          </div>
                        </td>
                        <td className="py-4 px-6">
                          <div>
                            <p className="text-sm font-medium text-gray-800">{booking.property.name}</p>
                            <p className="text-xs text-gray-500">{booking.property.room}</p>
                          </div>
                        </td>
                        <td className="py-4 px-6 text-sm text-gray-600">{booking.dates}</td>
                        <td className="py-4 px-6 text-sm font-bold text-gray-900 text-right">{booking.price}</td>
                        <td className="py-4 px-6">
                          <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-semibold ${getStatusBadgeColor(booking.status, booking.statusColor)}`}>
                            {booking.status}
                          </span>
              ...
              {/* Pagination - Updated to yellow/red theme */}
              <div className="flex items-center justify-between px-6 py-4 bg-gradient-to-r from-yellow-50 to-red-50 border-t border-gray-200">
                <p className="text-sm text-gray-500">
                  Showing <span className="font-semibold text-gray-900">1</span> to <span className="font-semibold text-gray-900">{filteredBookings.length}</span> of <span className="font-semibold text-gray-900">{bookings.length}</span> results
                </p>
                <div className="flex items-center gap-1">
                  <button className="p-2 rounded hover:bg-gray-200 disabled:opacity-30 disabled:hover:bg-transparent" disabled>
                    <span className="material-icons text-sm">chevron_left</span>
                  </button>
                  <button className="px-3 py-1 rounded bg-gradient-to-r from-yellow-500 to-red-500 text-white text-sm font-bold">1</button>
                  <button className="p-2 rounded hover:bg-gray-200 disabled:opacity-30" disabled>
                    <span className="material-icons text-sm">chevron_right</span>
                  </button>
                </div>
              </div>
      ...
      {/* Side Drawer (Booking Detail View) - Updated to yellow/red theme */}
      <aside className={`fixed right-0 top-0 h-full w-[450px] bg-white border-l border-gray-200 shadow-2xl z-40 transform transition-transform duration-300 ${isDrawerOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        {selectedBooking && (
          <div className="h-full flex flex-col">
            <div className="p-6 border-b border-gray-200 flex items-center justify-between bg-gradient-to-r from-yellow-50 to-red-50">
              <div>
                <h2 className="text-xl font-bold text-gray-900">Booking Details</h2>
                <p className="text-sm text-red-600 font-medium">{selectedBooking.id}</p>
              </div>
              <button 
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-8 bg-white">
              {/* Guest Section */}
              <div className="space-y-4">
                <h3 className="text-xs font-bold text-gray-400 uppercase tracking-widest">Guest Information</h3>
                <div className="flex items-center gap-4">
                  <img 
                  ...
                  />
                  <div>
                    <h4 className="text-lg font-bold text-gray-900">{selectedBooking.guest.name}</h4>
              ...
              {/* Property Details */}
              <div className="space-y-2">
                <h3 className="text-xs font-bold text-gray-400 uppercase tracking-widest">Property Details</h3>
                <div className="bg-gray-50 p-4 rounded-xl border border-gray-200">
                  <p className="font-medium text-gray-900">{selectedBooking.property.name}</p>
                  <p className="text-sm text-gray-500 mt-1">{selectedBooking.property.room}</p>
                  <div className="grid grid-cols-2 gap-4 mt-3 text-sm">
                    <div>
                      <span className="text-gray-500">Room Size:</span>
                      <span className="ml-2 font-medium text-gray-900">{selectedBooking.property.roomSize || 'N/A'} m²</span>
                    </div>
                    <div>
                      <span className="text-gray-500">Beds:</span>
                      <span className="ml-2 font-medium text-gray-900">{selectedBooking.property.beds || 'N/A'}</span>
                    </div>
                    <div>
                      <span className="text-gray-500">Max Occupancy:</span>
                      <span className="ml-2 font-medium text-gray-900">{selectedBooking.property.maxOccupancy || 'N/A'}</span>
                    </div>
                  </div>
                </div>
              </div>
              ...
              {/* Booking Timeline */}
              <div className="space-y-2">
                <h3 className="text-xs font-bold text-gray-400 uppercase tracking-widest">Booking Timeline</h3>
              ...
              {/* Financial Summary */}
              <div className="bg-gray-50 p-4 rounded-xl border border-gray-200 space-y-3">
                <h3 className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-2">Payment Summary</h3>
                <div className="flex justify-between text-sm">
                  <span className="text-gray-500">Total Amount</span>
                  <span className="text-gray-900 font-bold">{selectedBooking.price}</span>
                </div>
                <div className="flex justify-between text-sm">
                ...
                <div className="flex justify-between text-sm">
                  ...
                  <span className={`px-2 py-0.5 rounded-full text-xs font-semibold ${
                    selectedBooking.paymentStatus ? 'bg-yellow-100 text-yellow-700' : 'bg-orange-100 text-orange-700'
                  }`}>
                    {selectedBooking.paymentStatus ? 'Paid' : 'Pending'}
                  </span>
                </div>
                <div className="pt-3 border-t border-gray-200 flex justify-between">
                  ...
                  <span className="text-base font-bold text-red-600">{selectedBooking.price}</span>
                </div>
              </div>

              {/* Actions */}
              <div className="space-y-3 pt-4">
                <button className="w-full py-3 bg-gradient-to-r from-yellow-500 to-red-500 text-white rounded-xl font-bold hover:from-yellow-600 hover:to-red-600 transition-colors shadow-sm">
                  Download Invoice (PDF)
                </button>
                <button className="w-full py-3 border border-gray-200 text-gray-700 rounded-xl font-bold hover:bg-gray-50 transition-colors">
                  Send Message to Guest
                </button>
                {!selectedBooking.isbookingcancel && (
                  <button className="w-full py-3 border border-rose-200 text-rose-600 rounded-xl font-bold hover:bg-rose-50 transition-colors">
