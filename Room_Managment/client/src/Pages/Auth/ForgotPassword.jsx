  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 via-yellow-50 to-red-50 flex items-center justify-center p-4">
      ...
      <div className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-xl w-full max-w-md p-8 relative z-10 border border-yellow-100">
        ...
        {/* Progress Steps - Updated to yellow/red gradient */}
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center w-full">
            <div className={`flex items-center justify-center w-8 h-8 rounded-full ${
              step >= 1 ? 'bg-gradient-to-r from-yellow-500 to-red-500 text-white' : 'bg-gray-200 text-gray-600'
            }`}>
              1
            </div>
            <div className={`flex-1 h-1 mx-2 ${
              step >= 2 ? 'bg-gradient-to-r from-yellow-500 to-red-500' : 'bg-gray-200'
            }`}></div>
            <div className={`flex items-center justify-center w-8 h-8 rounded-full ${
              step >= 2 ? 'bg-gradient-to-r from-yellow-500 to-red-500 text-white' : 'bg-gray-200 text-gray-600'
            }`}>
              2
            </div>
            <div className={`flex-1 h-1 mx-2 ${
              step >= 3 ? 'bg-gradient-to-r from-yellow-500 to-red-500' : 'bg-gray-200'
            }`}></div>
            <div className={`flex items-center justify-center w-8 h-8 rounded-full ${
              step >= 3 ? 'bg-gradient-to-r from-yellow-500 to-red-500 text-white' : 'bg-gray-200 text-gray-600'
            }`}>
              3
            </div>
          </div>
          ...
          <form onSubmit={handleVerifyOTP} className="space-y-6">
            <div>
              ...
              <div className="flex justify-between gap-2 mb-4">
                {otp.map((digit, index) => (
                  <input
                    ...
                    onKeyDown={(e) => handleOtpKeyDown(index, e)}
                    onPaste={index === 0 ? handleOtpPaste : undefined}
                    className="w-12 h-14 text-center text-2xl font-bold border-2 border-gray-300 rounded-lg focus:border-yellow-500 focus:ring-2 focus:ring-yellow-200 outline-none transition-all"
                    disabled={loading}
                    autoFocus={index === 0}
                  />
                ))}
              ...
              <div className="flex items-center justify-between text-sm">
                {timer > 0 ? (
                  <p className="text-gray-600">
                    OTP expires in: <span className="font-mono font-bold text-red-500">{formatTime(timer)}</span>
                  </p>
                ) : (
                  <p className="text-red-600">OTP expired</p>
                )}
                ...
                <button
                  ...
                  onClick={handleResendOTP}
                  disabled={loading || resendDisabled}
                  className="text-yellow-600 hover:text-red-600 font-medium disabled:text-gray-400 disabled:cursor-not-allowed"
                >
                  {loading ? 'Sending...' : 'Resend OTP'}
                </button>
              </div>
        ...
        <div className="mt-6 text-center">
          <button
            onClick={() => window.location.href = 'mailto:support@roommanagement.com'}
            className="text-xs text-gray-500 hover:text-yellow-600 hover:underline"
          >
            Need help? Contact support
          </button>
        </div>
