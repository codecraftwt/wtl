...
const Profile = () => {
  ...
  const getProfilePhotoUrl = () => {
    ...
    // Fallback to UI Avatars with yellow background
    console.log('🎨 Using fallback avatar for:', user.name);
    return `https://ui-avatars.com/api/?name=${encodeURIComponent(user.name || 'User')}&background=EAB308&color=fff&size=128`;
  };
    ...
    <div className="max-w-6xl mx-auto px-4 py-6">
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        {!editMode ? (
          <button
            onClick={() => setEditMode(true)}
            className="px-4 py-2 text-sm bg-gradient-to-r from-yellow-500 to-red-500 text-white rounded-lg hover:from-yellow-600 hover:to-red-600 transition-all flex items-center gap-2 shadow-md hover:shadow-lg"
          >
            <span className="material-icons-outlined text-sm">edit</span>
            Edit Profile
          </button>
          ...
          <div className="flex gap-2">
            ...
            <button
              onClick={handleSaveChanges}
              disabled={updating}
              className="px-4 py-2 text-sm bg-gradient-to-r from-yellow-500 to-red-500 text-white rounded-lg hover:from-yellow-600 hover:to-red-600 disabled:opacity-50 transition-all flex items-center gap-2 shadow-md hover:shadow-lg"
            >
              {updating ? (
                <>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-6 shadow-sm">
            <h3 className="text-lg font-semibold text-gray-800 mb-4 flex items-center gap-2">
              <span className="material-icons-outlined text-yellow-500">person</span>
              Basic Information
            </h3>
            ...
            <div className="flex flex-col sm:flex-row gap-6">
              ...
              <div className="flex flex-col items-center sm:items-start">
                <div className="relative group">
                  <img
                    key={user.profilePhoto || 'fallback'} // Force re-render when photo changes
                    className="w-24 h-24 rounded-full border-4 border-yellow-200 shadow-sm object-cover"
                    alt="Profile"
                    src={getProfilePhotoUrl()}
                    onError={(e) => {
                      console.log('❌ Image failed to load:', e.target.src);
                      // If image fails to load, fallback to UI Avatars with yellow background
                      e.target.onerror = null; // Prevent infinite loop
                      e.target.src = `https://ui-avatars.com/api/?name=${encodeURIComponent(user.name || 'User')}&background=EAB308&color=fff&size=128`;
                    }}
                  />
                    ...
                    <>
                      <div className="absolute inset-0 bg-black bg-opacity-50 rounded-full opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                        <button
                          onClick={() => fileInputRef.current.click()}
                          disabled={uploadingPhoto}
                          className="text-white text-xs font-medium px-2 py-1 bg-gradient-to-r from-yellow-500 to-red-500 rounded hover:from-yellow-600 hover:to-red-600"
                        >
                          {uploadingPhoto ? '...' : 'Change'}
                        </button>
                      </div>
                ...
                {/* Uploading indicator */}
                {uploadingPhoto && (
                  <p className="mt-2 text-xs text-red-500">Uploading...</p>
                )}
              </div>
                ...
                <div>
                  ...
                  {editMode ? (
                    <input
                      ...
                      value={formData.name || ''}
                      onChange={handleInputChange}
                      className="mt-1 w-full text-lg font-semibold text-gray-800 bg-white border border-gray-200 rounded-lg px-3 py-2 focus:border-yellow-500 focus:ring-1 focus:ring-yellow-500 outline-none"
                      placeholder="Full Name"
                    />
                  ) : (
                    <p className="mt-1 text-lg font-semibold text-gray-800">{user.name || 'User Name'}</p>
                  )}
                </div>

                <div>
                  <label className="text-xs font-medium text-gray-500 uppercase tracking-wider">Email Address</label>
                  <p className="mt-1 text-sm text-gray-600">{user.email || 'email@example.com'}</p>
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-6 shadow-sm">
            <h3 className="text-lg font-semibold text-gray-800 mb-4 flex items-center gap-2">
              <span className="material-icons-outlined text-yellow-500">location_on</span>
              Location Information
            </h3>
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <select
                    value={selectedCountry}
                    onChange={handleCountryChange}
                    className="w-full px-3 py-2 border border-gray-200 rounded-lg focus:border-yellow-500 focus:ring-1 focus:ring-yellow-500 outline-none"
                    disabled={loadingLocation}
                  >
                    <option value="">Select Country</option>
                    {Array.isArray(countries) && countries.map((country) => (
                ...
                {loadingLocation && (
                  <p className="text-xs text-yellow-600">Loading location data...</p>
                )}
              </div>
            ) : (
              <div>
                ...
                {showMap && user.location?.latitude && user.location?.longitude && (
                  <div className="mt-4 border border-gray-200 rounded-lg overflow-hidden">
                    <div className="bg-gradient-to-r from-yellow-50 to-red-50 px-4 py-2 border-b border-gray-200">
                      <p className="text-xs font-medium text-gray-600">📍 Location Map</p>
                    </div>
                    <div 
                      id="profile-map" 
                      style={{ height: '300px', width: '100%' }}
          ...
          <div className="bg-white rounded-xl border border-gray-200 p-6 shadow-sm">
            <h3 className="text-lg font-semibold text-gray-800 mb-4 flex items-center gap-2">
              <span className="material-icons-outlined text-yellow-500">settings</span>
              Account Status
            </h3>
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                ...
                <button
                  ...
                  className={`relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-yellow-500 focus:ring-offset-2 ${
                    isActive ? 'bg-gradient-to-r from-yellow-500 to-red-500' : 'bg-gray-300'
                  }`}
                  role="switch"
                >
                  <span
              ...
              <div className="border-t border-gray-100 pt-4">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-sm font-medium text-gray-800">Account ID</p>
                    <p className="text-xs text-gray-500 font-mono">{user.id || user._id || 'Not available'}</p>
                  </div>
                </div>
              </div>
            </div>
        ...
        <div className="space-y-6">
            ...
            <div className="space-y-4">
              ...
              <div className="border-t border-gray-100 pt-4">
                <div className="flex items-center justify-between">
                  ...
                  <button 
                    onClick={() => setShowPasswordModal(true)}
                    className="text-sm text-yellow-600 hover:text-red-600 font-medium"
                  >
                    Change
                  </button>
                </div>
          ...
          <div className="bg-white rounded-xl border border-red-200 p-6 shadow-sm">
            ...
            <div className="space-y-2">
              <button className="w-full px-4 py-2.5 text-sm text-gray-700 hover:bg-gray-50 rounded-lg transition-colors flex items-center justify-between border border-gray-200">
                <span>Download your data</span>
                <span className="material-icons-outlined text-sm text-gray-400">download</span>
              </button>
              <button className="w-full px-4 py-2.5 text-sm text-red-600 hover:bg-red-50 rounded-lg transition-colors flex items-center justify-between border border-red-200">
                <span>Delete account</span>
                <span className="material-icons-outlined text-sm text-red-400">delete_forever</span>
              </button>
            </div>
            ...
            <div className="space-y-4">
              ...
              <div className="flex gap-3 pt-4">
                <button
                  onClick={updatePassword}
                  disabled={updating}
                  className="flex-1 px-4 py-2 bg-gradient-to-r from-yellow-500 to-red-500 text-white rounded-lg hover:from-yellow-600 hover:to-red-600 disabled:opacity-50 transition-all shadow-md hover:shadow-lg"
                >
                  {updating ? (
